...
Sidecar の再実装ではなく、「ScreenCaptureKit → VideoToolbox(H.264) → Wi-Fi → iPad でハードウェアデコード」という映像パスの実証が目的。
...
3. Mac と同じ Wi-Fi に接続していれば一覧に Mac 名が出るのでタップして接続
   - 初回はローカルネットワークの許可ダイアログが出る
...
- 音声は送らない
- 暗号化なし（家庭内 LAN 前提。`NWParameters.tcp` そのまま）
- 同時に受信できる iPad は 1 台
